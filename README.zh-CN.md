...
2. **打开终端**（可选）— 点击 **仅启动终端** 直接在项目目录打开 Shell
3. **选择分支** — Git 分支面板显示当前分支，启动前可直接切换
4. **选择会话模式**
   - `新会话` — 全新开始
   - `Continue` — 续接上次会话（`--continue`）
   - `Resume` — 从历史会话列表中选择一次恢复（`--resume`）
5. **配置模型** — 自动从 `.claude/settings.json` 读取，也可随时手动覆盖
6. **预览并启动** — 底部实时显示完整命令，点击 **复制** 或直接点 **启动**
